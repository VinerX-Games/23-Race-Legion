...
**HiveWE CLI** (`C:\Games\HiveWE_VinerX_Edition\build\Release\Release\HiveWE_cli.exe`) — головной CLI для работы с картой:
- `safe-move` — перемещает файл и правит все ссылки в `.w3u/.w3t/.w3a`/`war3map.lua`
- `build-map` — собирает `.w3x` из папки
- `search-objects` / `get-object` / `set-field` — работа с объектовыми данными
- `list-fields` / `list-all-races` / `describe-race` — инспекция рас и полей
- Сборка: `cmake --preset Release && cmake --build --preset Release`

Исходники HiveWE: `C:\Games\HiveWE_VinerX_Edition\` (форк stijnherfst/HiveWE)
