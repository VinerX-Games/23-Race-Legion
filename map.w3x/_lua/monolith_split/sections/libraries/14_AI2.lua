-- library AI2:
---@param p player
---@return nothing
function SetLimits(p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A0IQ'), p)
	SetPlayerAllianceStateBJ(p, Player(12), bj_ALLIANCE_UNALLIED)
	SetPlayerAllianceStateBJ(Player(12), p, bj_ALLIANCE_UNALLIED)
	--  -----------------------
	SetPlayerTechMaxAllowedSwap(FourCC('H048'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H03S'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H028'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H03H'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H018'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H05K'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H04S'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H053'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H04W'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H052'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H04X'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H05M'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H06B'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H05L'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H054'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H03J'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H06C'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H01K'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H01N'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H01J'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H01M'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H01L'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('O011'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N010'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N00Z'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U007'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0C4'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0C7'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0C5'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0C6'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U00O'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U015'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U01W'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U01V'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U01U'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U00V'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U00U'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U014'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E01W'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E01F'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E01D'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E01E'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E01C'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E025'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E024'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E026'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N02F'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N02G'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N02B'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N02A'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U027'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U028'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N046'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N047'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N045'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N040'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0EZ'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0F0'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N037'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0HP'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0HQ'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U014'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0H8'), 0, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0H9'), 0, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0H7'), 0, p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A05J'), p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A05K'), p)
	--  -----------------------
	SetPlayerAbilityAvailableBJ(false, FourCC('A07K'), p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A07L'), p)
	--  -----------------------
	SetPlayerAbilityAvailableBJ(false, FourCC('A07M'), p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A07S'), p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A07T'), p)
	--  -----------------------
	SetPlayerAbilityAvailableBJ(false, FourCC('A07D'), p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A02G'), p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A07O'), p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A07N'), p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A07P'), p)
	SetPlayerAbilityAvailableBJ(false, FourCC('A07Q'), p)
	--  -----------------------
	SetPlayerTechMaxAllowedSwap(FourCC('H043'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H044'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H05H'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H05I'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H045'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('Hjnd'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('W200'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('W201'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('W202'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N058'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('O031'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('O030'), 1, p)
	-- Bandits
	SetPlayerTechMaxAllowedSwap(FourCC('H047'), 1, p)
	-- Goblins
	SetPlayerTechMaxAllowedSwap(FourCC('H0BD'), 1, p)
	-- Naga
	SetPlayerTechMaxAllowedSwap(FourCC('H0JV'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0JU'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N07A'), 1, p)
	-- Stromgard
	SetPlayerTechMaxAllowedSwap(FourCC('H0HB'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0HL'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0HA'), 1, p)
	-- Ogres
	SetPlayerTechMaxAllowedSwap(FourCC('N05J'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N05K'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N05L'), 1, p)
	-- Pandarens
	SetPlayerTechMaxAllowedSwap(FourCC('PA36'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('PA37'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('PA38'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('PA40'), 1, p)
	-- Bezlikie
	SetPlayerTechMaxAllowedSwap(FourCC('U02H'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U02G'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U02I'), 1, p)
	-- JungleTrolls
	SetPlayerTechMaxAllowedSwap(FourCC('O054'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('O05A'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('O05D'), 1, p)
	-- Worgen
	SetPlayerTechMaxAllowedSwap(FourCC('H0J2'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0J6'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0J7'), 1, p)
	-- IceTrolls
	SetPlayerTechMaxAllowedSwap(FourCC('O04H'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('O04G'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('O04I'), 1, p)
	-- FelOrc
	SetPlayerTechMaxAllowedSwap(FourCC('N072'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N073'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('N06P'), 1, p)
	-- Ents
	SetPlayerTechMaxAllowedSwap(FourCC('E02Q'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E02R'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('E02S'), 1, p)
	-- Gnomes
	SetPlayerTechMaxAllowedSwap(FourCC('H0GC'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0GE'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('H0GG'), 1, p)
	-- Silitids
	SetPlayerTechMaxAllowedSwap(FourCC('U023'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U024'), 1, p)
	SetPlayerTechMaxAllowedSwap(FourCC('U025'), 1, p)
end
-- Принудительный выбор расы AI по текстовому токену (для команды "-aiN <раса>"
-- и CLI-агента). Возвращает true, если токен распознан и раса запущена.
-- Токены латиницей, регистр не важен.
---@param pi integer
---@param token string|nil
---@return boolean
function StartAiRaceByToken(pi, token)
	if token == nil or token == "" then
		ProbeLogWrite("[AI] StartAiRaceByToken pi=" .. tostring(pi) .. " token=nil -> random will be used")
		return false
	end
	token = string.lower(token)
	ProbeLogWrite("[AI] StartAiRaceByToken pi=" .. tostring(pi) .. " token=" .. tostring(token))
	local race = AiRaceByToken(token)
	if race == nil then
		ProbeLogWrite("[AI] StartAiRaceByToken pi=" .. tostring(pi) .. " token=" .. tostring(token) .. " unknown -> random will be used")
		return false
	end
	ProbeLogWrite("[AI] StartAiRaceByToken pi=" .. tostring(pi) .. " => " .. tostring(race.key))
	return AiStartRace(pi, race)
end
---@param pi integer
---@param raceToken string|nil   optional race override; nil/unknown = random
---@return nothing
function createAiPlayer(pi, raceToken)
	ProbeLogWrite("[AI] createAiPlayer ENTER pi=" .. tostring(pi) .. " raceToken=" .. tostring(raceToken or "nil"))
	gPlayer = Player(pi)
	TriggerExecute(gg_trg_TurnAi)
	ProbeLogWrite("[AI] createAiPlayer TurnAi executed")

	udg_AiControl[pi] = true
	ForceAddPlayerSimple(gPlayer, udg_Bots)
	AiBrainBotListAdd(pi)
	if AiApplyBotHandicap ~= nil then AiApplyBotHandicap(pi) end  -- bot HP/dmg advantage dials (neutral by default)
	ProbeLogWrite("[AI] createAiPlayer bot added to Bots force, count=" .. tostring(CountPlayersInForceBJ(udg_Bots)))
	--  Обычное появление и ресы

	-- Задаю место

	udg_LocalPoint = AiPickSpawnPoint(pi)
	ProbeLogWrite("[AI] createAiPlayer start location set")

	--  Раса аи (опциональный форс через raceToken, иначе случайно)
	if not StartAiRaceByToken(pi, raceToken) then
		local race = AiRacePickRandom()
		ProbeLogWrite("[AI] createAiPlayer random race pick=" .. tostring(race and race.key or "nil"))
		if not AiStartRace(pi, race) then
			ProbeLogWrite("[AI] createAiPlayer random race pick failed")
		end
	end
	ProbeLogWrite("[AI] createAiPlayer race=" .. tostring(AiRace[pi]) .. " set")
	
	-- R7: give bots a player-style capital (10000 HP, 30 armor, in StolicaGroups).
	-- Without this, bots have no capital → no death-on-capital-lost mechanic.
	-- Use GroupEnumUnitsOfPlayer because udg_Ai_buildings[pi] may be empty yet
	-- (join triggers fire asynchronously after race spawning).
	local race = AiRaceOf(pi)
	if race and race.buildings and race.buildings.seed then
		local seedType = race.buildings.seed
		local enumG = CreateGroup()
		GroupEnumUnitsOfPlayer(enumG, gPlayer, nil)
		local sz = BlzGroupGetSize(enumG)
		for i = 0, sz - 1 do
			local u = BlzGroupUnitAt(enumG, i)
			if u ~= nil and GetUnitTypeId(u) == seedType
				and GetUnitState(u, UNIT_STATE_LIFE) > 0.405 then
				MakeCapital(u)
				ProbeLogWrite("[AI] createAiPlayer capital set pi=" .. tostring(pi) .. " type=" .. tostring(seedType))
				break
			end
		end
		DestroyGroup(enumG)
	end
	
	udg_LocalText2 = GetPlayerName(gPlayer)
	udg_LocalText2 = (I2S(pi + 1) .. ". ") .. udg_LocalText2
	local ownerIndex = EnsureMultiboardPlayerRow(pi)
	if ownerIndex ~= nil then
		MultiboardSetItemValue(MultiboardItem[ownerIndex * 2 + 0], udg_LocalText2)
	end
	
	
	
	--  Создает юнита с аурами
	BonusUnit[pi] = CreateUnitAtLoc(gPlayer, FourCC('aiu0'), udg_LocalPoint, bj_UNIT_FACING)
	UnitAddAbility(BonusUnit[pi], FourCC('aib0'))
	UnitAddAbility(BonusUnit[pi], FourCC('aib1'))
	ProbeLogWrite("[AI] createAiPlayer bonus unit created")
	
	CreateArmyBonusUnit(gPlayer)
	
	-- Лимиты
	SetLimits(gPlayer)
	AiLimitsSet()
	ProbeLogWrite("[AI] createAiPlayer limits set AiRepeat=" .. tostring(AiRepeat) .. " AiMass=" .. tostring(AiMass) .. " AiLimit=" .. tostring(AiLimit))
	
	
	SetPlayerStateBJ(gPlayer, PLAYER_STATE_RESOURCE_GOLD, 5000)
	SetPlayerStateBJ(gPlayer, PLAYER_STATE_RESOURCE_LUMBER, 5000)
	SetBools()
	TriggerExecute(gg_trg_CityNearWater2)
	
	--  Данные ИИшки
	--  Таймеры ИИ
	
	-- TimerSmall (builders): disabled in brain mode; BrainBuild handles it
	if not AiBrainEnabled(pi) then
		StartTimerBJ(udg_TimerSmall, false, 1.11 * AiRepeat / 5)
		ProbeLogWrite("[AI] createAiPlayer TimerSmall started period=" .. tostring(1.11 * AiRepeat / 5))
		StartTimerBJ(udg_TimerSmall2, false, 2.12 * AiRepeat / 5)
		ProbeLogWrite("[AI] createAiPlayer TimerSmall2 started period=" .. tostring(2.12 * AiRepeat / 5))
		-- TimerSmall4 (navy join): disabled in brain mode; BrainNavalFocus handles it
		StartTimerBJ(udg_TimerSmall4, false, 4.14 * AiRepeat / 5)
		ProbeLogWrite("[AI] createAiPlayer TimerSmall4 started period=" .. tostring(4.14 * AiRepeat / 5))
		else
		-- Brain mode: start PlayerGet1 directly (round-robin, no ForcePickRandom)
		TimerStart(udg_PlayerGet1, 0.8 * AiRepeat / 5, true, nil)
		ProbeLogWrite("[AI] createAiPlayer PlayerGet1 started directly (brain round-robin) period=" .. tostring(0.8 * AiRepeat / 5))
	end
	StartTimerBJ(udg_TimerSmall3, false, 3.13 * AiRepeat / 5)
	ProbeLogWrite("[AI] createAiPlayer TimerSmall3 started period=" .. tostring(3.13 * AiRepeat / 5))
	StartTimerBJ(udg_AiTimerStrateg, true, 15.15 * AiRepeat / 5)
	ProbeLogWrite("[AI] createAiPlayer AiTimerStrateg started period=" .. tostring(15.15 * AiRepeat / 5))
	StartTimerBJ(udg_TimerToChangeAi, false, 600.00)
	TimerStart(aiFixer, 1800.00, true, nil)
	ProbeLogWrite("[AI] createAiPlayer DONE pi=" .. tostring(pi) .. " race=" .. tostring(AiRace[pi]) .. " AiRepeat=" .. tostring(AiRepeat))
end
-- library AI2 ends
