...
### Режимы ожидания результата

По умолчанию агент опрашивает наличие файла результата каждые 200ms (`time.sleep(0.2)`).

**`--watch`** — включает ожидание через WinAPI `ReadDirectoryChangesW`: агент
**не опрашивает**, а блокируется до появления файла в папке `CustomMapData`.
Реакция микросекундная, нет слепых задержек. Это полезно для интерактивных
сессий (ИИ шлёт eval и ждёт мгновенного ответа).

Без `--watch` — классический polling (200ms). Надёжно, но добавляет до 200ms
латентности на каждый запрос. Подходит для скриптов/пакетных вызовов.
