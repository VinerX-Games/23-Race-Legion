...
Скрипты живут в `scripts/` + два ключевых в корне.
...
| Файл | Описание |
|---|---|
| `../build_map_lua.py` | **Основной сборочный скрипт** (корень). Собирает `map.w3x/war3map.lua` из split-исходников (`_lua/monolith_split/`) по `manifest.json`. Флаги: `--check-only` — проверка SHA256 без перезаписи. |
...
## AI-ДАННЫЕ И ВАЛИДАЦИЯ

| Файл | Описание |
|---|---|
| `validate_ai_races.py` | **Валидатор определений AI-рас.** Читает `82_ai_races.lua` + `82_ai_races/*.lua` и проверяет: required fields (tokens, weight, altar, start), дубликаты FourCC ID, gate/branch-ссылки (определены ли в таблице `gates`/`branches`), production-здания (есть ли в buildings), ecoWeights, legacy-фоллбэки, неиспользуемые gates, конфликты токенов. Выдаёт сводную таблицу. |
| `split_ai_races_bundle.py` | Раcщепляет `82_ai_races.lua` на отдельные per-race файлы в `82_ai_races/`. Извлекает блоки `RegisterAiRace("Имя", {...})`, сохраняет в `NN_имя.lua`, обновляет `manifest.json`. |
| `migrate_workers.py` | Мигрирует 22 AI-расы с процедурного production-table worker на декларативную секцию `worker = { id, cap, from }`. Удаляет старые production-записи для town-hall зданий, вставляет новый worker-блок. Пропускает сложные расы (Scarlet, ForestTrolls, Goblins). |
...
| Файл | Описание |
|---|---|
| `../agent_bridge.py` | **Live Agent Bridge** (корень). Исполняет произвольный Lua в работающей карте WC3 через eval-канал (Preloader-файлы). Режимы: `exec "code"`, `exec --file`, `--watch` (мгновенный ответ), `reset` (очистка inbox/out/hb). Heartbeat-синхронизация v2. |
| `audit.py` | Анализирует паттерны `Condition()` в runtime-файле. |

---

## СКРИПТЫ ВНУТРИ MAP.W3X

| Файл | Описание |
|---|---|
| `../map.w3x/_lua/monolith_split/sections/update_manifest.py` | Обновляет `manifest.json` при изменениях в секциях (v1). |
| `../map.w3x/_lua/monolith_split/sections/update_manifest2.py` | То же (v2). |
| `../map.w3x/_lua/monolith_split/sections/update_manifest3.py` | То же (v3). |
