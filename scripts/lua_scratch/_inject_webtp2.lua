-- Hot-load the IMPROVED web-portal mass-TP (cluster-based, web-route) and drive it.
AiWebPortalSpecs = {
...
AiWebPortals=nil; AiWebPortalGraph=nil; AiWebPortalCast={}
function AiBuildWebPortalCache()
  if AiWebPortals~=nil then return end
  AiWebPortals={}; AiWebPortalGraph={}
  for _,spec in ipairs(AiWebPortalSpecs) do
    ...
    if u and r then
      ...
      if sc and dc and sc~=dc then
        AiWebPortals[#AiWebPortals+1]={unit=u,rect=r,x=px,y=py,radius=spec[3],srcCont=sc,dstCont=dc}
        local nb=AiWebPortalGraph[sc]; if nb==nil then nb={}; AiWebPortalGraph[sc]=nb end; nb[dc]=true
      end
    end
  end
end
function AiWebRoute(src,dst)
  ...
  local q={{node=src,path={src}}}; local vis={[src]=true}; local h=1
  while h<=#q do local cur=q[h]; h=h+1; local nb=AiWebPortalGraph[cur.node]
    if nb then for n,_ in pairs(nb) do
      if n==dst then local p={}; for _,v in ipairs(cur.path) do p[#p+1]=v end; p[#p+1]=dst; return p end
      if not vis[n] then vis[n]=true; local np={}; for _,v in ipairs(cur.path) do np[#np+1]=v end; np[#np+1]=n; q[#q+1]={node=n,path=np} end
    end end
  end
  return nil
end
function AiFindWebPortal(s,d,x,y)
  AiBuildWebPortalCache(); local best,bd
  for _,wp in ipairs(AiWebPortals) do
    if wp.srcCont==s and wp.dstCont==d and GetUnitState(wp.unit,UNIT_STATE_LIFE)>0.405 then
      local dx,dy=x-wp.x,y-wp.y; local dd=dx*dx+dy*dy; if bd==nil or dd<bd then best,bd=wp,dd end end
  end
  return best
end
function AiPortalTeleport(pi,portal,rect,radius)
  ...
  while i<sz and moved<150 do local u=BlzGroupUnitAt(army,i); i=i+1
    if u and GetUnitState(u,UNIT_STATE_LIFE)>0.405 and not IsUnitType(u,UNIT_TYPE_STRUCTURE)
      ...
      if dx*dx+dy*dy<=r2 then SetUnitPosition(u,GetRandomReal(minx,maxx),GetRandomReal(miny,maxy)); moved=moved+1 end end
  end
  return moved
end
AiWebMinArmy=16; AiWebGatherFrac=0.40; AiWebMinGathered=10; AiWebCooldownTicks=16
function BrainWebPortalTick(pi,p,wm)
  if wm==nil or wm.defendHome then return "p"..pi..":defend/nil" end
  ...
  local oc=AiContinentOf(focus.x,focus.y); if oc==nil then return "p"..pi..":oc nil" end
  local sz=BlzGroupGetSize(army); local buckets={}; local mil=0
  for i=0,sz-1 do local u=BlzGroupUnitAt(army,i)
    if u and GetUnitState(u,UNIT_STATE_LIFE)>0.405 and not IsUnitType(u,UNIT_TYPE_STRUCTURE) and not IsUnitType(u,UNIT_TYPE_PEON) then
      local ux,uy=GetUnitX(u),GetUnitY(u); local c=AiContinentOf(ux,uy)
      if c then local b=buckets[c]; if b==nil then b={n=0,sx=0,sy=0}; buckets[c]=b end; b.n=b.n+1; b.sx=b.sx+ux; b.sy=b.sy+uy; mil=mil+1 end end end
  if mil<AiWebMinArmy then return "p"..pi..":army "..mil.."<min (obj "..oc..")" end
  local bestCont,bestN
  for c,b in pairs(buckets) do if c~=oc and (bestN==nil or b.n>bestN) then bestCont,bestN=c,b.n end end
  if bestCont==nil then return "p"..pi..":all on obj "..oc end
  local b=buckets[bestCont]; if b.n<AiWebMinGathered then return "p"..pi..":biggest off-obj "..bestCont.."="..b.n.."<min" end
  local cx,cy=b.sx/b.n,b.sy/b.n
  local rt=AiWebRoute(bestCont,oc); local nextCont=(rt and #rt>=2) and rt[2] or oc
  local rs=""; if rt then local s={}; for _,v in ipairs(rt) do s[#s+1]=v end; rs=table.concat(s,">") else rs="NIL" end
  local wp=AiFindWebPortal(bestCont,nextCont,cx,cy)
  if wp==nil then return "p"..pi..":no webportal "..bestCont.."->"..tostring(nextCont).." route="..rs end
  ...
  if within<need then return "p"..pi..":cluster "..bestCont.."="..b.n.." gathering "..within.."/"..need.." route="..rs end
  local moved=AiPortalTeleport(pi,wp.unit,wp.rect,wp.radius)
  return "p"..pi..":MASS-TP moved="..moved.." "..bestCont.."->"..wp.dstCont.." route="..rs.." (obj "..oc..")"
end
local res={}
for pi=1,16 do
